...
def plot_matches_for_images(data, image, images) -> None:
    ...
    for im1, im2 in pairs:
        ...
        i1 = data.load_image(im1, anydepth=True)
        i2 = data.load_image(im2, anydepth=True)
        i1 = cv2.normalize(i1, i1, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
        i2 = cv2.normalize(i2, i2, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
        plot_matches(i1, i2, p1, p2)
        i += 1
        if args.save_figs:
